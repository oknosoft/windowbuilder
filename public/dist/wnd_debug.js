;(function (root, factory) {
  ...
  $p.CatFurns = class CatFurns extends $p.CatFurns {
    ...
    add_furn_prm(aprm = [], afurn_set = []) {
      ...
      this.selection_params.forEach((row) => {
  ...
  $p.CatFurnsSpecificationRow = class CatFurnsSpecificationRow extends $p.CatFurnsSpecificationRow {
    ...
    check_restrictions(contour, cache) {
      ...
      if (!cache.ignore_formulas && !formula.empty() && formula.condition_formula && !formula.execute({
        ox: cache.ox,
        contour,
        row_furn: this
      })) {
      ...
      function furn_check_opening_restrictions(contour, cache) {
        ...
        furn.open_tunes.forEach((row) => {
          ...
          if (len < lmin || len > lmax || angle < amin || (angle > amax && amax > 0) || (!elm.is_linear() && !row.arc_available)) {
            new_spec_row({
              elm,
              row_base: {clr: $p.cat.clrs.get(), nom: $p.job_prm.nom.furn_error},
              origin: furn,
              spec,
              ox
            });
      ...
      function base_spec_profile(elm) {
        ...
        if (row_cnn) {
          ...
          row_spec = new_spec_row({
            ...
            row_base: row_cnn,
            nom: _row.nom,
            origin: cnn_row(_row.elm, prev ? prev.elm : 0),
            spec,
            ox
          });
  ...
  $p.DocCalc_order = class DocCalc_order extends $p.DocCalc_order {
      ...
      return this.product_rows(true)
        .then(() => {
          return this._manager.pouch_db
            .query('linked', {
              startkey: [this.ref, 'cat.characteristics'],
              endkey: [this.ref, 'cat.characteristics\u0fff']
            })
            .then(({rows}) => {
    ...
    process_add_product_list(dp) {
      ...
      dp.production.forEach((row_dp) => {
        ...
        if (row_dp.inset.empty()) {
          ...
          res = res
            .then(() => this.create_product_row({
              row_spec: row_dp,
              elm,
              len_angl,
              params: dp.product_params,
              create: true
            }))
            .then((row_prod) => {
    ...
    static set_department() {
      ...
      if (current_user && this.department.empty() || this.department.is_new()) {
        current_user.acl_objs && current_user.acl_objs.find_rows({
          by_default: true,
          type: cat.divisions.class_name
        }, (row) => {
          ...
          oxml: {
            ' ': [
              {id: 'number_doc', path: 'o.number_doc', synonym: 'Номер', type: 'ro'},
              {
                ...
                synonym: 'Дата',
                type: 'ro',
                txt: moment(o.date).format(moment._masks.date_time)
              },
              'number_internal'
            ],
        ...
        wnd.elmnts.pg_right = wnd.elmnts.cell_right.attachHeadFields({
          ...
          oxml: {
            ...
            'Итоги': [{
              id: 'doc_currency',
              path: 'o.doc_currency',
              synonym: 'Валюта документа',
              type: 'ro',
              txt: o['doc_currency'].presentation
            },
              {id: 'doc_amount', path: 'o.doc_amount', synonym: 'Сумма', type: 'ron', txt: o['doc_amount']},
              {
                id: 'amount_internal',
                path: 'o.amount_internal',
                synonym: 'Сумма внутр',
                type: 'ron',
                txt: o['amount_internal']
              }]
          }
        });
  ...
  (({adapters: {pouch}, classes, cat, doc, job_prm, md, pricing, utils}) => {
    ...
    function on_log_in() {
      ...
      if (pouch.props.direct) {
        ...
        pouch.local.templates = new classes.PouchDB('templates', {
          adapter: 'idb',
  ...
  WndAddressData.fias = {
    ...
    "1020": {
      ...
      order: 2,
      fid: 1,
      syn: [" вл.", " вл ", " влад.", " влад ", " владен.", " владен ", " владение"]
    },
    ...
    "1070": {
      ...
      order: 4,
      fid: 2,
      syn: [" соор.", " соор ", " сооруж.", " сооруж ", "сооружение"]
    },
